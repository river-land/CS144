...
void do_test_2( const TCPConfig& cfg, default_random_engine& rd )
{
  ...
  const Wrap32 rx_isn( rd() ); // ISN随机值
  TCPReceiverTestHarness test_2 { "overlapping out-of-order segments", cfg.recv_capacity }; //测试名，容量大小65000
  test_2.execute( SegmentArrives {}.with_seqno( rx_isn ).with_syn() );//发出SYN包，给出ISN号
  vector<tuple<size_t, size_t>> seq_size;
  size_t datalen = 0;
  while ( datalen < cfg.recv_capacity ) { //累计给出的数据长度，小于容量时循环
    const size_t size = min( 1 + ( static_cast<size_t>( rd() ) % ( TCPConfig::MAX_PAYLOAD_SIZE - 1 ) ),//随机给出一个包的大小，控制要比剩余容量小
                             cfg.recv_capacity - datalen );
    const size_t rem = TCPConfig::MAX_PAYLOAD_SIZE - size; //相对于最大装载包，当前包所占用的剩余大小
    size_t offs = 0; //偏移量0
    if ( rem == 0 ) { //如果当前随机产生的包，有1000大小
      offs = 0;
    } else if ( rem == 1 ) { //如果是999大小
...
int main()
{
  try {
    ...
    for ( unsigned rep_no = 0; rep_no < NREPS; ++rep_no ) {
      do_test_2( cfg, rd );
      std::cout<<"successssssssssssssssssssssssssssssssssssssssss"<<std::endl;
    }
