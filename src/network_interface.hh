...
const int32_t initial_TTL = 30000;

struct AddressHasher{
  std::size_t operator()(const Address& add)const{
    return add.ipv4_numeric();
  }
};
...
private:
  ...
  std::unordered_map<Address,std::pair<EthernetAddress,int32_t>,AddressHasher> AddressMapping{};
  std::queue<EthernetFrame> EthernetQueue{};
  std::queue<std::pair<InternetDatagram,Address>> InternetDatagramQueue{};
  bool send_ARP_in_five_secs = 0;
  size_t real_time = 0;
  size_t send_ARP_time = 0;
